...
import React, { type ComponentProps, useRef, useState, useEffect, ReactNode } from "react";
import { Button, Tooltip, Badge } from "@heroui/react";
import { sanitizeHtml } from "@/utils/security";
import "./ln-button.scss";
...
export interface LnButtonProps
  ...
  /** Icon hiển thị kèm theo (ReactNode đơn lẻ hoặc mảng các IconSlots) */
  icon?: ReactNode | IconSlots[];
  /** Vị trí hiển thị Icon khi truyền dưới dạng ReactNode đơn lẻ (Mặc định: 'left') */
...
const LnButton: React.FC<LnButtonProps> = ({
  ...
  const renderLeftIcons = () => {
    ...
    if (Array.isArray(icon)) {
      return icon
        .filter((item) => (item.iconPosition ?? "left") === "left")
        .map((item, index) => <React.Fragment key={index}>{item.icon}</React.Fragment>);
    }
  ...
  const renderRightIcons = () => {
    ...
    if (Array.isArray(icon)) {
      return icon
        .filter((item) => item.iconPosition === "right")
        .map((item, index) => <React.Fragment key={index}>{item.icon}</React.Fragment>);
    }
